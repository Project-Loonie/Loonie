...
VERSION=
commit=false
url=https://github.com/Project-Loonie/Loonie
proc=2
mem=2000
lxc=true
...
Run this script from the directory containing the loonie, gitian-builder, gitian.sigs, and loonie-detached-sigs.
...
-u|--url	Specify the URL of the repository. Default is https://github.com/Project-Loonie/Loonie
...
then
	# Make output folder
	mkdir -p "./loonie-binaries/${VERSION}"
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit loonie=${COMMIT} --url loonie=${url} ../loonie/contrib/gitian-descriptors/gitian-linux.yml
	    ./bin/gsign --signer $SIGNER --release ${VERSION}-linux --destination ../gitian.sigs/ ../loonie/contrib/gitian-descriptors/gitian-linux.yml
	    mv build/out/loonie-*.tar.gz build/out/src/loonie-*.tar.gz ../loonie-binaries/${VERSION}
	fi
	# Windows
	if [[ $windows = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit loonie=${COMMIT} --url loonie=${url} ../loonie/contrib/gitian-descriptors/gitian-win.yml
	    ...
	    mv build/out/loonie-*.zip build/out/loonie-*.exe ../loonie-binaries/${VERSION}
	fi
	# Mac OSX
	if [[ $osx = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit loonie=${COMMIT} --url loonie=${url} ../loonie/contrib/gitian-descriptors/gitian-osx.yml
	    ...
	    mv build/out/loonie-*.tar.gz build/out/loonie-*.dmg ../loonie-binaries/${VERSION}
	fi
	# AArch64
	if [[ $aarch64 = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit loonie=${COMMIT} --url loonie=${url} ../loonie/contrib/gitian-descriptors/gitian-aarch64.yml
	    ./bin/gsign --signer $SIGNER --release ${VERSION}-aarch64 --destination ../gitian.sigs/ ../loonie/contrib/gitian-descriptors/gitian-aarch64.yml
	    mv build/out/loonie-*.tar.gz build/out/src/loonie-*.tar.gz ../loonie-binaries/${VERSION}
	fi
	popd || exit
...
then
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=${COMMIT} ../loonie/contrib/gitian-descriptors/gitian-win-signer.yml
	    ...
	    mv build/out/loonie-*win64-setup.exe ../loonie-binaries/${VERSION}
	    mv build/out/loonie-*win32-setup.exe ../loonie-binaries/${VERSION}
	fi
	# Sign Mac OSX
	if [[ $osx = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=${COMMIT} ../loonie/contrib/gitian-descriptors/gitian-osx-signer.yml
	    ...
	    mv build/out/loonie-osx-signed.dmg ../loonie-binaries/${VERSION}/loonie-${VERSION}-osx.dmg
	fi
	popd || exit
