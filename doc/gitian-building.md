...
*Setup instructions for a Gitian build of Loonie using a VM or physical system.*
...
- [Building Loonie](#building-loonie-core)
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/gitianuser/gitian-builder/inputs/loonie/.git/
...
./bin/gbuild --commit loonie=${COMMIT} --url loonie=${URL} ../loonie/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit loonie=${COMMIT} --url loonie=${URL} ../loonie/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit loonie=${COMMIT} --url loonie=${URL} ../loonie/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the loonie git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../loonie/contrib/gitian-descriptors/*|sort|uniq )
...
./bin/gbuild --url loonie=${BTCPATH},signature=${SIGPATH} ../loonie/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/loonie-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/loonie-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/loonie-osx-build.assert
```
...
[loonieproject/gitian.sigs](https://github.com/LoonieCoin/gitian.sigs/) repository, or if that's not possible create a pull
