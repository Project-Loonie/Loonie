...
static std::string ValueToString(CAmount nValue, bool AllowNegative = false)
{
    ...
    QString Str = BitcoinUnits::formatWithUnit(BitcoinUnits::LNI, nValue);
...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown) {
        ...
        if (!GetBoolArg("-txindex", false)) {
            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (loonie.conf).");
            QMessageBox::warning(this, "Loonie Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}
...
{
    QString CSS = "body {font-size:12px; color:#ccffff; bgcolor:#4F5F98;}\n a, span { font-family: monospace; }\n span.addr {color:#4F5F98; font-weight: bold;}\n table tr td {padding: 3px; border: 1px solid black; background-color: #4F5F98;}\n td.d0 {font-weight: bold; color:#f8f6f6;}\n h2, h3 { white-space:nowrap; color:#4F5F98;}\n a { color:#00ccff; text-decoration:none; }\n a.nav {color:#4F5F98;}\n";
