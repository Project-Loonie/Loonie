                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zLNI ought to be enough for anybody." - Bill Gates, 2017
    ui->zLNIpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzLNISyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zLNIpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzLNI_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zLNI is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zLNI...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zLNI in ") +
...
void PrivacyDialog::on_pushButtonSpendzLNI_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zLNI is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zLNI
        sendzLNI();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zLNI
    sendzLNI();
}

void PrivacyDialog::on_pushButtonZLNIControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ZLNIControlDialog* zLNIControl = new ZLNIControlDialog(this);
    zLNIControl->setModel(walletModel);
    zLNIControl->exec();
}

void PrivacyDialog::setZLNIControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzLNISelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzLNI()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!IsValidDestinationString(ui->payTo->text().toStdString())) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Loonie Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zLNIpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zLNI is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " LNI </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zLNI</b>";
    ...
    // use mints from zLNI selector if applicable
    vector<CMintMeta> vMintsToFetch;
    vector<CZerocoinMint> vMintsSelected;
    if (!ZLNIControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZLNIControlDialog::GetSelectedMints();
        ...
        for (auto& meta : vMintsToFetch) {
            if (meta.nVersion < libzerocoin::PrivateCoin::PUBKEY_VERSION) {
                ...
                if (nSecurityLevel < 100) {
                    QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zLNI require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
                    ui->TEMintStatus->setPlainText(tr("Failed to spend zLNI"));
                    ui->TEMintStatus->repaint();
                    return;
                }
            }
    ...
    if (!fSuccess) {
        if (receipt.GetStatus() == ZLNI_SPEND_V1_SEC_LEVEL) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zLNI require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
            ui->TEMintStatus->setPlainText(tr("Failed to spend zLNI"));
            ui->TEMintStatus->repaint();
            return;
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zLNI was spent successfully update the addressbook with the label
    ...
    ZLNIControlDialog::setSelectedMints.clear();
    ui->labelzLNISelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zLNI Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " LNI, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zLNI Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zLNI </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zLNI "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zLNI "));
    ui->labelzLNIAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzLNISyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    bool fButtonsEnabled =  ui->pushButtonMintzLNI->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zLNI
        ui->pushButtonMintzLNI->setEnabled(false);
        ui->pushButtonMintzLNI->setToolTip(tr("zLNI is currently disabled due to maintenance."));

        // Spend zLNI
        ui->pushButtonSpendzLNI->setEnabled(false);
        ui->pushButtonSpendzLNI->setToolTip(tr("zLNI is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zLNI
        ui->pushButtonMintzLNI->setEnabled(true);
        ui->pushButtonMintzLNI->setToolTip(tr("PrivacyDialog", "Enter an amount of LNI to convert to zLNI", 0));

        // Spend zLNI
        ui->pushButtonSpendzLNI->setEnabled(true);
        ui->pushButtonSpendzLNI->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}