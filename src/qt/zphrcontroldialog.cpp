...
std::set<std::string> ZLNIControlDialog::setSelectedMints;
std::set<CMintMeta> ZLNIControlDialog::setMints;

ZLNIControlDialog::ZLNIControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZLNIControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZLNIControlDialog::~ZLNIControlDialog()
{
    delete ui;
}

void ZLNIControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZLNIControlDialog::updateList()
{
...
void ZLNIControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZLNIControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZLNI_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZLNIControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZLNIControlDialog::GetSelectedMints()
{
...
void ZLNIControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
