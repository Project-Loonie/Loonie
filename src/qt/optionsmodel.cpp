...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::LNI);
    ...
    if (!settings.contains("nAnonymizeLoonieAmount"))
        settings.setValue("nAnonymizeLoonieAmount", 1000);

    nAnonymizeLoonieAmount = settings.value("nAnonymizeLoonieAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeLoonieAmount"))
        SoftSetArg("-anonymizeloonieamount", settings.value("nAnonymizeLoonieAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in loonie.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeLoonieAmount:
            return QVariant(nAnonymizeLoonieAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeLoonieAmount:
            nAnonymizeLoonieAmount = value.toInt();
            settings.setValue("nAnonymizeLoonieAmount", nAnonymizeLoonieAmount);
            emit anonymizeLoonieAmountChanged(nAnonymizeLoonieAmount);
            break;
        case CoinControlFeatures:
