...
public:
    ...
    bool WriteZLNISeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZLNISeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZLNISeed_deprecated(uint256& seed);
    bool EraseZLNISeed();
    bool EraseZLNISeed_deprecated();

    bool WriteZLNICount(const uint32_t& nCount);
    bool ReadZLNICount(uint32_t& nCount);
