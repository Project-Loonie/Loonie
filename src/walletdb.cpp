...
bool CWalletDB::WriteZLNISeed(const uint256& hashSeed, const vector<unsigned char>& seed)
{
...
bool CWalletDB::EraseZLNISeed()
{
    uint256 hash;
    if(!ReadCurrentSeedHash(hash)){
        return error("Failed to read a current seed hash");
    }
    if(!WriteZLNISeed(hash, ToByteVector(base_uint<256>(0) << 256))) {
...
bool CWalletDB::EraseZLNISeed_deprecated()
{
    return Erase(string("dzs"));
}

bool CWalletDB::ReadZLNISeed(const uint256& hashSeed, vector<unsigned char>& seed)
{
    return Read(make_pair(string("dzs"), hashSeed), seed);
}

bool CWalletDB::ReadZLNISeed_deprecated(uint256& seed)
{
    return Read(string("dzs"), seed);
}

bool CWalletDB::WriteZLNICount(const uint32_t& nCount)
{
    return Write(string("dzc"), nCount);
}

bool CWalletDB::ReadZLNICount(uint32_t& nCount)
{
