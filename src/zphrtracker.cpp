...
CzLNITracker::CzLNITracker(std::string strWalletFile)
{
...
CzLNITracker::~CzLNITracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzLNITracker::Init()
{
...
bool CzLNITracker::Archive(CMintMeta& meta)
{
...
bool CzLNITracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzLNITracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzLNITracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzLNITracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzLNITracker::GetSerialHashes()
{
...
CAmount CzLNITracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzLNITracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzLNITracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzLNITracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzLNITracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzLNITracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzLNITracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzLNITracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzLNITracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzLNITracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzLNITracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzLNITracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzLNITracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzLNITracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzLNITracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzLNITracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzLNITracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
