...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 LNI at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zLNI send
enum ZerocoinSpendStatus {
    ZLNI_SPEND_OKAY = 0,                            // No error
    ZLNI_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZLNI_WALLET_LOCKED = 2,                         // Wallet was locked
    ZLNI_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZLNI_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZLNI_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZLNI_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZLNI_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZLNI_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZLNI_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZLNI_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZLNI_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZLNI_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZLNI_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZLNI_SPENT_USED_ZLNI = 14,                      // Coin has already been spend
    ZLNI_TX_TOO_LARGE = 15,                         // The transaction is larger than the max tx size
    ZLNI_SPEND_V1_SEC_LEVEL
};
...
public:
    ...
    void ZLNIBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZLNIOutput(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    */
    ...
    CzLNIWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzLNITracker> zphrTracker;
    ...
    void setZWallet(CzLNIWallet* zwallet)
    {
        zwalletMain = zwallet;
        zphrTracker = std::unique_ptr<CzLNITracker>(new CzLNITracker(strWalletFile));
    }

    CzLNIWallet* getZWallet() { return zwalletMain; }
    ...
    void setZLNIAutoBackups(bool fEnabled)
    {
