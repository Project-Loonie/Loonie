...
BOOST_AUTO_TEST_CASE(test_checkpoints)
{
    ...
    BOOST_CHECK_MESSAGE(AccumulatorCheckpoints::mapCheckpoints.at(7474)
                                .at(libzerocoin::CoinDenomination::ZQ_FIVE_THOUSAND)
                                .GetHex() == "33343834376237393132356438393037633666623561626630343033386235366231626637653834313030373430656538353230326238363736626566323034", "does not match");
}
...
BOOST_AUTO_TEST_CASE(deterministic_tests)
{
    ...
    CzLNIWallet zWallet(wallet.strWalletFile);
    ...
    for (int i = 0; i < nTests; i++) {
        ...
        zWallet.GenerateDeterministicZLNI(denom, coin, dMint);
