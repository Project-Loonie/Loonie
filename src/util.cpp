...
int nAnonymizeLoonieAmount = 1000;
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "loonie" is a composite category enabling all Loonie-related debug output
            if (ptrCategory->count(string("loonie"))) {
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "loonie.conf"));
    ...
    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty loonie.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override loonie.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "loonied.pid"));
