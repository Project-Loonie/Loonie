...
CzLNIWallet::CzLNIWallet(std::string strWalletFile)
{
    ...
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZLNISeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZLNISeed_deprecated()) {
                    LogPrintf("%s: Updated zLNI seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
...
bool CzLNIWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZLNICount(nCountLastUsed);
    else if (!walletdb.ReadZLNICount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzLNIWallet::Lock()
{
    seedMaster = 0;
}

void CzLNIWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzLNIWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZLNI(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzLNIWallet::LoadMintPoolFromDB()
{
...
void CzLNIWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzLNIWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzLNIWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzLNIWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZLNI(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CzLNIWallet::SeedToZLNI(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzLNIWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzLNIWallet::GenerateDeterministicZLNI(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzLNIWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzLNIWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
